...
func LoadFAgent(e *ecs.ECS, row, col int) {
	...
	fagentTicker := &FAgentTicker{tankEntry: tankEntry, CurrentTick: 0, CompleteTick: 60, State: "WAITING", ecs: e}
	component.Ticker.SetValue(tankEntry, component.DummyTicker{fagentTicker})

}

type FAgentTicker struct {
...
// this routine will behave similar like tank but it can move vertically
func (c *FAgentTicker) Tick() {
	var tankVX = 14.0
	var tankVZ = tankVX / 2
	c.CurrentTick += 3
	...
	if c.CurrentTick == c.CompleteTick {
		...
		case "WAITING":
			...
			if vTank.X == 0 {
				targetZ := playerPos.Z + float64(4*component.GridLength)
				targetZ = math.Min(float64(component.GridStartPointY)+4*float64(component.GridWidth), targetZ)
				targetZ = math.Max(float64(component.GridStartPointY), targetZ)
				direction = targetZ - pos.Z
				//vTank := component.Velocity.Get(c.tankEntry)
				if direction < 0 {
					vTank.Z = tankVZ
					vTank.Y = tankVZ
			...
			if vTank.X == 0 && vTank.Y == 0 {
				c.State = "WARMUP"
				c.CompleteTick = 90
				component.Sprite.Get(c.tankEntry).Image = assets.FAgent2
			}
		...
		case "WARMUP":
			c.State = "ATTACK"
			c.CompleteTick = 27
			component.Sprite.Get(c.tankEntry).Image = assets.FAgent3
		...
		case "ATTACK":
			c.State = "WAITING"
			c.CompleteTick = 60
			component.Sprite.Get(c.tankEntry).Image = assets.FAgent1
		}
		c.CurrentTick = 0
		//c.CompleteTick = 100
	}
